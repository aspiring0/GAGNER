...
class CoPredictor(nn.Module):
    def __init__(self, cls_num, hid_size, biaffine_size, channels, ffnn_hid_size, dropout=0):
        ...
        self.dropout = nn.Dropout(dropout)
        # Dynamic Gating Layer (Core Modification Section)
        self.gate = nn.Sequential(
            nn.Linear(2 * cls_num, 64),  #  Input dimension: 2*C (concatenation of o1 and o2)
            nn.LayerNorm(64),  # Stability Training
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(64, 2),  # Output two weight values
        )
        self._init_gate_weights()  # Customized Initialization
    def _init_gate_weights(self):
        """Initialize the gating layer: 
        Set the weights of the last layer to zero, 
        and set the biases to equal weights."""
        nn.init.zeros_(self.gate[-1].weight)  # The final layer of Linear
        nn.init.constant_(self.gate[-1].bias, 0.5)  # Initial Bias Average Fusion
    ...
    def forward(self, x, y, z):
        ...
        # return gate1 * o1 + gate2 * o2
        # return o1 + o2

        assert o1.shape == o2.shape
        B, L, _, C = o1.shape

        # Concatenate the last dimension (channel dimension) of o1 and o2.
        gate_input = torch.cat([o1, o2], dim=-1)  # [B, L, L, 2C]

        # Generate gated weights (Softmax ensures the sum of weights equals 1)
        gate_weights = F.softmax(self.gate(gate_input), dim=-1)  # [B, L, L, 2]

        # Weighted Fusion (Broadcast Mechanism)
        final_output = (
                gate_weights[..., 0:1] * o1 +  # The weighting component must maintain its dimensionality：[B,L,L,1] * [B,L,L,C]
                gate_weights[..., 1:2] * o2
        )  # [B, L, L, C]
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.bert_weights = nn.Parameter(torch.randn(4) * 0.1)
        lstm_input_size = 0
        ...
        self.predictor = CoPredictor(config.label_num,
    ...
    def forward(self, bert_inputs, grid_mask2d, dist_inputs, pieces2word, sent_length):
        ...
        self_attn = nn.MultiheadAttention(embed_dim=self.lstm_hid_size, num_heads=4)
        self_attn.cuda()
        attn_output, _ = self_attn(word_reps, word_reps, word_reps)  # [B, L, D]
        word_reps = word_reps + attn_output
